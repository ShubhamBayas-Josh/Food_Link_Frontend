...
const fetchData = async (): Promise<FoodTransaction[]> => {
  const token = localStorage.getItem("token");
  const response = await fetch("http://127.0.0.1:3000/api/v1/food_transactions", {
    headers: {
...
const TransactionList = () => {
  ...
  const [currentUserId, setCurrentUserId] = useState<number | null>(getUserIdFromToken());
          ...
          {isError && (
            <div className="bg-red-50 border-l-4 border-red-500 p-4 rounded">
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-red-700">Error loading donations: {error?.message}</p>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {!isLoading &&
              ...
              foodTransactions.map((item: FoodTransaction) => {
                ...
                const canClaim =
                  ["available", "pending"].includes(item.status?.toLowerCase()) && !expired;
                    ...
                    <div className="p-6">
                      ...
                      <div className="mt-4 pt-4 border-t border-gray-100">
                        <div className="flex justify-between items-center">
                          <span className="text-sm font-medium text-gray-600">Expiration info</span>
